...
function bind() {
    ...
    q('#edContent').addEventListener('touchend', () => { setTimeout(() => { saveSel(); updateToolbarState(); }, 50); });
    ...
    q('#tbHead').addEventListener('change', function () {
        ...
        q('#edContent').focus(); saveSel(); scheduleSave(); updateToolbarState();
    });
...
function showEditor() {
    ...
    if (S.mobile) q('#list').classList.add('hidden');
}
...
function onResize() {
    ...
    if (!S.mobile) {
        q('#list').classList.remove('hidden');
        q('#editor').classList.add('visible');
        if (S.fullscreen && wasMobile) toggleFullscreen();
    } else {
...
function applyPixelSize(px) {
    ...
    if (range.collapsed) {
        ...
        span.style.fontSize = px + 'px'; span.innerHTML = '\u200B';
...
function renderFolders() {
    ...
    S.folders.forEach((f, idx) => {
        ...
        d.dataset.fid = f.id; d.dataset.idx = idx;
        d.innerHTML = `
            <span class="fl-dot" style="background:${f.color}"></span>
            <span class="fl-name">${esc(f.name)}</span>
            <span class="fl-cnt">${cnt}</span>
            <div class="fl-acts">
                <button class="fl-ab fe" title="Edit"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>
                <button class="fl-ab dl fd" title="Delete"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
            </div>`;
        ...
        const onDown = (e) => {
            ...
            S.drag.startY = clientY; S.drag.moved = false; isDragging = false;
            pressTimer = setTimeout(() => { isDragging = true; dragStart(e, d, idx); }, 300);
            const onMoveCheck = (ev) => {
                const cy = ev.touches ? ev.touches[0].clientY : ev.clientY;
                if (Math.abs(cy - S.drag.startY) > S.drag.threshold && !isDragging) {
                    clearTimeout(pressTimer); isDragging = true; dragStart(e, d, idx);
                }
            };
            const onUp = () => {
                ...
                if (!isDragging && !S.drag.active) setFolderFilter(f.id, f.name);
            };
...
function dragStart(e, el, idx) {
    ...
    e.preventDefault();
    const container = q('#fList'), rect = el.getBoundingClientRect();
    const clientY = e.touches ? e.touches[0].clientY : e.clientY;
    const clone = el.cloneNode(true);
    clone.classList.add('fl-dragging');
    clone.style.width = rect.width + 'px'; clone.style.top = rect.top + 'px'; clone.style.left = rect.left + 'px';
    document.body.appendChild(clone);
    const ph = document.createElement('div');
    ph.className = 'fl-placeholder'; ph.style.height = rect.height + 'px';
    el.parentNode.insertBefore(ph, el); el.classList.add('fl-hidden');
    S.drag = {
        active: true, el, clone, ph, offsetY: clientY - rect.top,
        startIdx: idx, curIdx: idx, startY: S.drag.startY,
        threshold: S.drag.threshold, moved: true,
        items: Array.from(container.querySelectorAll('.fl-item:not(.fl-hidden)')).map((item, i) => {
...
function dragEnd() {
    ...
    S.drag.clone.style.top = phRect.top + 'px'; S.drag.clone.style.opacity = '0.6';
    setTimeout(() => {
        S.drag.clone.remove(); S.drag.ph.remove(); S.drag.el.classList.remove('fl-hidden');
        const { startIdx, curIdx } = S.drag; S.drag.active = false;
...
async function reorderFolders(from, to) {
    ...
    S.folders.splice(to, 0, moved); renderFolders();
    try {
...
function filtered() {
    ...
    if (S.search) {
        ...
        ns = ns.filter(n => (n.title || '').toLowerCase().includes(s) || strip(n.content || '').toLowerCase().includes(s));
    }
    switch (S.filter) {
...
function updateNav() {
    qa('.sb-link').forEach(b => b.classList.toggle('active', b.dataset.f === S.filter && S.filter !== 'folder'));
}
...
function applyColor(c) {
    ...
    if (S.cpMode === 'text') { document.execCommand('foreColor', false, c); q('#tcPreview').style.background = c; }
        ...
        else { document.execCommand('hiliteColor', false, c); q('#hlPreview').style.background = c; }
    }
